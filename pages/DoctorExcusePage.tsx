...
import DoctorExcuseForm from '../components/DoctorExcuseForm';
import DoctorExcusePreview from '../components/DoctorExcusePreview';
import TemplateChooser from '../components/TemplateChooser';
import PreviewAndExport from '../components/PreviewAndExport';
import { medicalTemplates } from '../templates/templates';
import type { DoctorExcuseData, Template, PatientData } from '../types';
...
const initialFormData: DoctorExcuseData = {
  patientFirstName: '',
  patientMiddleName: '',
  patientLastName: '',
  patientSuffix: '',
  ...
  returnDate: '',
  diagnosis: '',
  aiReasonForAbsence: '', // New field for AI-generated vague reason
  hospitalName: '',
  hospitalAddress: '',
  hospitalPhone: '',
  hospitalUrl: '',
...
const APP_STORAGE_KEY = 'doctorExcuseFormData_v4'; // Version bump for new fields and logic
...
interface DoctorExcusePageProps {
  onGoHome: () => void;
}

const DoctorExcusePage: React.FC<DoctorExcusePageProps> = ({ onGoHome }) => {
  const [formData, setFormData] = useState<DoctorExcuseData>(() => {
    try {
      const savedData = localStorage.getItem(APP_STORAGE_KEY);
      if (savedData) {
          const parsed = JSON.parse(savedData);
          // Merge with initialFormData to ensure new fields are present if not in saved data
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof DoctorExcuseData, string>>>({});
  ...
  const handleHospitalSelect = async (hospital: Hospital) => {
    ...
    try {
      ...
      .catch(err => {
          ...
          setFetchPhysiciansError("Could not fetch physicians. You can still enter one manually.");
          return [];
      });
  ...
  const handleNextFromForm = () => {
    const validationErrors = validateDoctorExcuse(formData);
        ...
        return (
          <>
            <div className="flex-grow overflow-y-auto custom-scrollbar-dark">
                <DoctorExcuseForm formData={formData} setFormData={setFormData} physicians={physicians} physicianError={fetchPhysiciansError} errors={errors} setErrors={setErrors} />
            </div>
      ...
      case 'template':
        return (
          <TemplateChooser
            ...
            onBack={() => setStep('form')}
            onCancel={onGoHome}
            PreviewComponent={DoctorExcusePreview}
          />
        );
      case 'preview':
        return (
            <PreviewAndExport
                ...
                PreviewComponent={DoctorExcusePreview}
                fileName={`doctors_excuse_${formData.patientLastName || 'patient'}`}
            />
        );
      default:
        return null;
...
export default DoctorExcusePage;